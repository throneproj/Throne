...
namespace Configs {
    ...
    RouteRuleDeprecated::RouteRuleDeprecated() {
    ...
    RouteRuleDeprecated::RouteRuleDeprecated(const RouteRuleDeprecated& other) {
        name = other.name;
    ...
    QJsonObject RouteRuleDeprecated::get_rule_json(bool forView, const QString& outboundTag) {
        QJsonObject obj;
    ...
    QStringList RouteRuleDeprecated::get_attributes()
    {
        return {
            "ip_version",
            "network",
    ...
    inputType RouteRuleDeprecated::get_input_type(const QString& fieldName) {
        if (fieldName == "invert" ||
    ...
    QStringList RouteRuleDeprecated::get_values_for_field(const QString& fieldName) {
        if (fieldName == "ip_version") {
            return {"", "4", "6"};
        }
    ...
    QStringList RouteRuleDeprecated::get_current_value_string(const QString& fieldName) {
        if (fieldName == "ip_version") {
            return {ip_version};
        }
        if (fieldName == "network") {
    ...
    QString RouteRuleDeprecated::get_current_value_bool(const QString& fieldName) const {
    ...
    void RouteRuleDeprecated::set_field_value(const QString& fieldName, const QStringList& value) {
        if (fieldName == "ip_version") {
            ip_version = value[0];
        }
        if (fieldName == "network") {
    ...
    bool RouteRuleDeprecated::isEmpty() {
    ...
    std::shared_ptr<RouteRuleDeprecated> RouteRuleDeprecated::get_processPath_direct_rule(QString processPath)
    {
        auto res = std::make_shared<RouteRuleDeprecated>();
    ...
    QList<std::shared_ptr<RouteRuleDeprecated>> RoutingChain::parseJsonArray(const QJsonArray& arr, QString* parseError) {
        if (arr.empty()) {
            parseError->append("Input is not a valid json array");
            return {};
        }

        auto rules = QList<std::shared_ptr<RouteRuleDeprecated>>();
        auto ruleID = 1;
        for (const auto& item: arr) {
            if (!item.isObject()) {
            ...
            auto rule = std::make_shared<RouteRuleDeprecated>();
    ...
    std::shared_ptr<RoutingChain> RoutingChain::GetDefaultChain() {
        ...
        auto defaultRule = std::make_shared<RouteRuleDeprecated>();
    ...
    QString RoutingChain::UpdateSimpleRules(const QString& content, simpleAction action)
    {
        ...
        QList<std::shared_ptr<RouteRuleDeprecated>> toRemove;
    ...
    bool RoutingChain::add_simple_rule(const QString& content, const std::shared_ptr<RouteRuleDeprecated>& rule, ruleType type)
    {
        if (type == simpleAddress) return add_simple_address_rule(content, rule);
        else return add_simple_process_rule(content, rule, type);
    }

    bool RoutingChain::add_simple_address_rule(const QString& content, const std::shared_ptr<RouteRuleDeprecated>& rule)
    {
    ...
    bool RoutingChain::add_simple_process_rule(const QString& content, const std::shared_ptr<RouteRuleDeprecated>& rule, ruleType type)
    {
    ...
    RoutingChain::RoutingChain(const RoutingChain& other)  : JsonStore(other) {
        ...
        for (const auto& item: other.Rules) {
            Rules.push_back(std::make_shared<RouteRuleDeprecated>(*item));
        }
        fn = QString(other.fn);
